...
const Dashboard = () => {
    ...
    const loadData = async () => {
      try {
        ...
        const [statsRes, recentRes, monthlyRes] = await Promise.all([
          ...
          fetch(
            `https://rrh-backend.vercel.app/api/dashboard/recent-bookings?limit=5${
              dateParams
                ? `&start=${dateParams.start}&end=${dateParams.end}`
                : ""
            }`
        ...
        setDashboardData({
          ...
          monthlyBookings: monthlyData.success
            ? monthlyData.data
            : { labels: [], counts: [] },
        });
      } catch (error) {
  ...
  const chartData = dashboardData.monthlyBookings.labels.map(
    (label, index) => ({
      month: label,
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm text-gray-600">
                  Track your booking trends over time
                </p>
              </div>
            </div>
          </div>
                  ...
                  {dashboardData.recentBookings.length === 0 ? (
                    ...
                    dashboardData.recentBookings.map((booking) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-4 text-sm text-gray-900 hidden lg:table-cell">
                          {booking.check_in || "—"}
                        </td>
                        <td className="py-4 px-4 text-sm text-gray-900 hidden lg:table-cell">
                          {booking.check_out || "—"}
                        ...
                        <td className="py-4 px-4">
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getBadgeStyles(
                              getPaymentBadgeVariant(booking.payment_status)
                            )}`}
                          >

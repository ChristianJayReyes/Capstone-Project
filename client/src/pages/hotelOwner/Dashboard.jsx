...
const Dashboard = () => {
  const [dateRange, setDateRange] = useState('all'); // 'week', 'month', 'last_month', 'year', 'all'
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: dashboardData.stats.pending_bookings,
      icon: Clock,
    ...
    {
      title: 'Confirmed',
      value: dashboardData.stats.confirmed_bookings,
      icon: CheckCircle2,
    ...
    {filterButtons.map((btn) => (
      <button
        ...
        onClick={() => {
          ...
          setShowCustomRange(false); // hide calendar if open
        }}
                  ...
                  {dashboardData.recentBookings.length === 0 ? (
                    ...
                    dashboardData.recentBookings.map((booking) => (
                      <tr key={booking.booking_id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-4 px-4">
                          <a 
                            href={`/owner/bookings?booking_id=${booking.booking_id}`}
                            className="inline-flex items-center justify-center px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-md transition-colors"
                          >
                            View
                          </a>
                        </td>

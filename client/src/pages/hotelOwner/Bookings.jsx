...
/**
 * NOTE about paymentStatus:
 * If your backend returns a payment field (e.g. payment_status), it'll be used.
 * Otherwise we infer:
 *  - if booking.status in ['confirmed','checked-in','checked-out'] => 'reserved'/'paid'
 *  - else => 'not-paid'
 */
...
const mapBookingData = (b) => {
  ...
  // If backend supplies a payment_status prefer that
  ...
  return {
    ...
    room: {
      ...
      pricePerNight: Number(b.price_per_night ?? b.price_per_night ?? 0),
...
const Bookings = () => {
  ...
  const refetch = async () => {
    try {
      const res = await fetch('http://localhost:8000/api/bookings/getBookings.php');
  ...
  // helper: load available rooms for a booking (by room_type_id and dates)
  const loadAvailableRooms = async (booking) => {
    setAvailableRooms([]);
    setSelectedRoomId(null);

    // you need an endpoint that returns available rooms for a given room_type and date range.
    // Example requested URL: /api/rooms/getAvailableRooms.php?room_type_id=1&check_in=2025-10-01&check_out=2025-10-03
    // If you don't have it, create it server-side. I assume it returns an array of { room_id, room_number, price_per_night, capacity_adults, capacity_children, status }
    try {
      const rtId = booking.room.roomTypeId ?? booking.raw.room_type_id;
      const url = new URL('http://localhost:8000/api/rooms/getAvailableRooms.php');
  ...
  const API_URL = 'http://localhost:8000/api/bookings/updateBookingStatus.php';
    try {
      if (action === 'openRooms') {
      ...
      if (action === 'confirm') {
        // open confirm modal (double-check)
        if (!selectedRoomId) {
  ...
  if (result.success) {
    ...
    alert('Confirm failed: ' + (result.message || 'unknown'));
  }
  return;
}
...
if (action === 'cancel') {
  ...
  if (result.success) {
    ...
    alert('Cancel failed: ' + (result.message || 'unknown'));
  }
  return;
}
...
if (action === 'doCheckin') {
  ...
  if (result.success) {
    ...
    alert('Check-in failed: ' + (result.message || 'unknown'));
  }
  return;
}
...
if (action === 'doCheckout') {
  ...
  if (result.success) {
    ...
    alert('Check-out failed: ' + (result.message || 'unknown'));
  }
  return;
}
